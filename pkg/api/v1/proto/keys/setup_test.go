...
func setupServerAndClient(t *testing.T, keyStore store.Key) keys.ServiceClient {
	t.Helper()

	srv := grpc.NewServer()
	keys.RegisterServiceServer(srv, keys.NewService(keyStore))

	const bufSize = 1024 * 1024
	lis := bufconn.Listen(bufSize)
	go func() {
		if err := srv.Serve(lis); err != nil {
			assert.Fail(t, "keys service server error", err)
		}
	}()
	...
	return keys.NewServiceClient(conn)
}
