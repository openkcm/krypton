...
func TestUnsafe_ExportKeyLatestVersion(t *testing.T) {
	...
	versions := []struct {
		...
		{"v1", []byte("key-material-version-1-padding!")},
		{"v2", []byte("key-material-version-2-padding!")},
		{"v3", []byte("key-material-version-3-padding!")},
	}
...
func TestUnsafe_DestroyKey(t *testing.T) {
	...
	// then
	assert.NoError(t, err)
	assert.ElementsMatch(t, []string{"v1", "v2", "v3"}, destroyResp.DestroyedVersions)
