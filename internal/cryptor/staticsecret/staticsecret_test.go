...
func TestStaticSecret_New(t *testing.T) {
	...
	for _, tt := range tts {
		t.Run(tt.name, func(t *testing.T) {
			// when
			subj, err := staticsecret.NewStaticSecret(tt.nameArg, tt.keyArg)
...
func TestStaticSecret_Encrypt(t *testing.T) {
	...
	subj, err := staticsecret.NewStaticSecret(cryptor.InfoNameStaticSecret, newSecretKey(t))
	require.NoError(t, err)
...
func TestStaticSecret_EncryptDecrypt(t *testing.T) {
	...
	t.Run("should fail to decrypt with wrong key", func(t *testing.T) {
		...
		subj1, err := staticsecret.NewStaticSecret(cryptor.InfoNameStaticSecret, newSecretKey(t))
		require.NoError(t, err)
